...
class TextView < View
  ...
  def initialization_objc
    if @name.end_with?('Label')
      "_#{name} = [UILabel new];"
    elsif @name.end_with?('Edit')
      "_#{name} = [UITextField new];"
    elsif @name.end_with?('Text')
      "_#{name} = [UITextView new];"
    end
  end
...
eh = ERB.new(File.read("./view_iOS.h"), nil, '-').result
em = ERB.new(File.read("./view_iOS.m"), nil, '-').result
